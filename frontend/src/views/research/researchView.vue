  ...
  <div>
    ...
    <div style="display:flex; justify-content: space-between; align-items: center; background-color: #F7F7F7; padding: 20px 10px;">
      ...
      <div style="font-size:17px;">
        <h3>가장 많이 사용한 곳</h3>
        <p v-for="(data, idx) in spend_data.filter((_, idx) => idx < key)" :key="idx">ㆍ{{ data.category }} {{ data.amount }} 원  </p>
      </div>
    </div>
    ...
    <div style="margin-top:55px;">
      ...
      <div v-for="recom_card in recom_cards.filter((_, idx) => idx < 3)" :key="recom_card.id" class="cell" v-on:click="this.$router.push('/research/card/' + recom_card.id)" style="background-color:#E7E7E7; border-color:#E7E7E7 ;">
        <div style="display: flex; justify-content: space-between; background-color:white; padding: 10px; margin: 5px;">
          ...
          <div>
            <div style="display:flex; font-weight: bold; overflow:hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 100px;">
              {{ recom_card.name }}
            </div>
